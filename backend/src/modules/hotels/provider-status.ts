...
export function getHotelProviderStatuses(): HotelProviderStatus[] {
  return [
    ...
    {
      provider: 'OpenStreetMap/Overpass Details',
      capabilities: ['DETAILS'],
      configured: true,
      implemented: true,
      status: 'READY',
    ...
    {
      provider: 'Staying API',
      capabilities: ['OFFERS'],
      configured: hasSecret(env.STAYING_API_KEY),
      implemented: true,
      status: hasSecret(env.STAYING_API_KEY) ? 'READY' : 'MISSING_API_KEY',
      requiredEnv: ['STAYING_API_KEY'],
      nextPhase: 8,
    },
    {
